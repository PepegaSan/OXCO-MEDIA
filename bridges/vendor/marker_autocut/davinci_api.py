...
def append_dict_for_subclip(
    ...
    Use only ``mediaPoolItem``, ``startFrame``, and ``endFrame`` for reliable
    subclips (half-open end frame in API space).
    ...
    **Compilation (multiple files, one timeline):** build one list of these dicts
    ...
    Do not advance ``recordFrame`` in **source** frames when batching a compilation
    (use one ``AppendToTimeline`` call for the list of dicts).
